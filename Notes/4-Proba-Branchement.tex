...
q_{n+1} = f_{Z_{n+1}}(0) = f_X \circ f_{Z_n} (0) = f_X(\Pr_1\{Z_n = 0\}) = f_X(q_n).
$$
La convergence de la suite $(q_n)_{n \geq 0}$ vers le plus petit point fixe s'obtient en remarquant que
$$
s < s^* \; \Rightarrow \; f(s) > s, \qquad \qquad
s > s^* \; \Rightarrow \; f(s) < s
$$
et en appliquant la récurrence précédente :
$$
\begin{tabular}{ccc}
  & $q_0 = .05$ & $q_0 = .75$ \\
  \begin{tabular}{lc}
...
%     \textcolor{blue}{\bf--} : première bissectrice \\ ~ \\
    \textcolor{blue}{\bf --} : $(q_n)_{n \geq 0}$
    %itérations du point fixe
  \end{tabular}
  &
  \begin{tabular}{cc}
  \includegraphics[width=.4\textwidth, trim=0 10 25 50, clip=]{ExGaltonWatson-fixPoint-q05}
  \end{tabular}
  &
  \begin{tabular}{cc}
  \includegraphics[width=.4\textwidth, trim=0 10 25 50, clip=]{ExGaltonWatson-fixPoint-q075}
  \end{tabular}
\end{tabular}
$$

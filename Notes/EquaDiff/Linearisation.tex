...
%-------------------------------------------------------------------------------
 \subsubsection*{Cas diagonalisable}
%-------------------------------------------------------------------------------
...
\remark
La proposition \ref{prop:odeLineaireDiagonalisable} dit que, au prix du changement de base $z = P^{-1} y$, le système $\dot y = A y$ est équivalent à $n$ équations différentielles linéaires unidimensionnelles ($\dot z = D z$).
...
La solution obtenue à la  proposition \ref{prop:odeLineaireDiagonalisable} s'écrit ainsi
$$
E(t) = \exp(t D) 
\qquad \text{et} \qquad 
...
En notant $z_0 = P^{-1} y_0$, la condition initiale $y_0$ se décompose sous la forme
$$
y_0 = P z_0 = \sum_i z_{i0} v_i
$$
où les $v_i$ sont les vecteur propres de $A$, i.e. les vecteurs colonnes de $P$. 

On peut ensuite étudier le comportement en temps long du système en considérant
\begin{align*}
    \lim_{t \to \infty} y(t)    & = \lim_{t \to \infty} \exp(t A) y_0 
...
\end{itemize}
 En conclusion, s'il existe au moins un $\lambda_i \geq 0$, un perturbation autour d'un point d'équilibre $y^* \in \ker(A)$ éloignera le système de $y^*$ dans la direction $v_i$. A l'opposé, si toutes les $\lambda_i$ sont strictement négatives, quelque soit $y_0$, $\lim_{t \to \infty} y(t) = 0$ {\sl qui est donc le seul équilibre stable}.

%-------------------------------------------------------------------------------
 \subsubsection*{Cas non diagonalisable}
%-------------------------------------------------------------------------------

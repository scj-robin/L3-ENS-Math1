...
# Stabilité
sapply(1:statNb, function(i){JacobODE2(y=statPoint2[i, ], parm=parm2)})
sapply(1:statNb, function(i){eigen(JacobODE2(y=statPoint2[i, ], parm=parm2))$values})
sapply(1:statNb, function(i){eigen(JacobODE2(y=statPoint2[i, ], parm=parm2))$vectors})
...
# y0 <- c(1, 1)
# # tMax <- 30; tStep <- 1e-3; tGrid <- seq(0, tMax, by=tStep); tNb <- length(tGrid)
# model <- function(t, y, parm){return(list(Lorenz2(y, parm=parm)))}
# traj2 <- ode.2D(y=y0, times=tGrid, func=model, parm=parm2, dimens=c(1, 1), method='ode45')

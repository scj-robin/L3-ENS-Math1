...
\remark
\begin{enumerate}
 \item On toujours choisir des vecteurs propres de norme 1 $(\|v_i\| = 1$). 
 \item Puisque les vecteurs propres sont orthogonaux deux à deux, les colonnes de la matrice de passage $P$ constituent alors une base orthonormée de $\Rbb^n$. 
...
\begin{proposition*}[1.4.8: Itérée d'un matrice diagonalisable]
  Si est $A \in \Mcal_k$ est diagonalisable ($A = P D P^{-1}$) et telle que sa plus grande valeur propre en module $\lambda_1$ soit d'ordre multiplicité 1 (on parle alors de valeur propre {\em dominante} ou de Perron-Frobénius) :
  $$
...
\bigskip
\progres{Début Cours 4 :
\begin{enumerate}[\itemdot]
 \item Matrice diagonalisable $A = P D P^{-1}$
 \item Matrice positive $A \succcurlyeq 0$ ($A \succ 0$) admet des val. propres $\geq 0$ ($> 0$).
 \item Matrice positive symétrique (ex.: matrice de variance) diagonalisable avec vec. propres othorgonaux
 \item Itérée d'un matrice diagonalisable avec valeur propre dominante : $\lim_{n \rightarrow \infty} \lambda_1^{-n} A^n = v_1 u_1^\top$
\end{enumerate}
}
...
\begin{theorem*}[Perron-Frobenius]
  ...
  \begin{itemize}
    ...
    \item Les vecteurs propres $v$ et $u_1$ associés à $\lambda_1$ respectivement à droite et à gauche : 
    $$

...
On considère $n$ points $x_1, \dots x_n$ de $[a, b]^d$ et on cherche à définir un point central $m$ du ``nuage de points'' $(x_i)_{1 \leq i \leq n}$. On notera $(x_{ij})_{1 \leq j \leq d}$ les coordonnées points $x_i$. 
...
On rappelle qu'une fonction $h: [a, b]^d \mapsto \Rbb$ est convexe si et seulement si pour tout couple $(u, v)$ et tout réel $0 < a < 1$, on a
$$
...
\begin{enumerate}
  \setcounter{enumi}{1}  
  \item Montrer qu'une fonction $h$ continue et strictement convexe admet un minimum unique.
  \solution{

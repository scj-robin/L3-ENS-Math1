...
\begin{enumerate}
  ...
  = \frac{M(y)}{\|y\|},
  $$  
  or $\|y\|$ est non nul, donc $M(y) = 0$ par continuité des applications linéaires (en $0_n$, en l'occurrence).
\end{enumerate}
\eproof
...
\paragraph*{Rappel.} La matrice jabobienne au point $x$ d'une application $f : \Rbb^n \mapsto \Rbb^k$, notée $J_xf$, est la matrice de $\Mcal_{k, n}$ dont l'élément $(i, j)$ vaut la dérivée de la fonction $f_i$ par rapport à la coordonnée $x_i$
$$
...
%   =   
  ...
  $$
  \frac{\partial f_i(x_1, \dots x_n)}{\partial x_j}
  $$
  vaut précisément $[D_xf(e_j)]_i$.
\end{enumerate}
...
%     \item Exemple : calcul de $J_x f$ pour le passage en coordonnées polaires \warning{démontrer la différentiabilité de $f$ : cf classeur.}
%     \item Composition : 
%     $$
...
%     $$
%     \warning{donner l'exemple de simple juste après.}

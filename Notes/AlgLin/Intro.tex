...
\end{align*}
soit \warning{Les colonnes contribuent aux lignes ($j \to i$) : orientation inverse des chaînes de Markov.}
$$
...
\begin{definition}[Matrice jacobienne]
  ...
  La matrice jacobienne de $f$ au point $x$, noté $J_x(f)$ est la matrice de $\Mcal_{k, n}$ de terme générique \warning{Les fonctions correspondent aux lignes : pour $k = 1$, vecteur jacobien = transposée du gradient.}
  $$

...
%-------------------------------------------------------------------------------
  On considère un processus de Bienaymé-Galton-Watson partant d'une population de taille 1 et dans laquelle le nombre de descendants par individu suit une loi géométrique $\Gcal(a)$ :
  $$
  \Pr\{X = k\} = (1-a) a^k
  \qquad \text{pour } k \geq 0.
  $$
  \begin{enumerate}
    \item Montrer que le nombre moyen de descendants vaut $m = a / (1 - a)$.
    \solution{On a
    $$
    m = \Esp(X) 
    ...
    Alternativement, on sait que $f_X(s) = (1-a)/(1-as)$, donc
    $f'_X(s) = a(1-a)/(1-as)^2$, soit $m = f'_X(1) = a/(1-a)$.}

...
\begin{enumerate}
  ...
  \solution{
  Si $N = 1$, on suit une unique population et donc $p = 1 - q$. 
  }
  %
  \item Déterminer $p$ si $N$ est distribué comme $X$. 
  Commenter.
  \solution{
  Si $N \overset{\Delta}{=} X$, on a $g = f$ et donc
  $$
  p = 1 - g(q) = 1 - f(q) = 1 - q
  $$
  puisque $q$ est un point fixe de $f$ ($f(q) = q$). Ce résultat n'est pas surprenant puisque ce cas correspond au premier décalé d'une génération.}
\end{enumerate}

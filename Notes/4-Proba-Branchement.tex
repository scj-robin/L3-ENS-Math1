...
\remarks
\begin{enumerate}
  \item Cette récurrence nous assure notamment que, en notant $m = \Esp(X_{ni}) = f'_X
  (1)$, $$
  ...
  $$
  f_{Z_n} = \underset{n \text{ fois}}{\underbrace{f_X \circ f_X \circ \dots \circ f_X}} \circ f_{Z_0}.
  $$
\end{enumerate}
...
m = \Esp(X) = f'_X(1)
$$
par rapport à 1 contrôle le nombre de points fixes de $f_X$.
...
\remarks
\begin{enumerate}
  \item Dans le lemme \ref{lem:BGWnombrePointsFixes}, on écarte le cas $f_x(0) = \Pr\{X = 0\} = 0$, car alors chaque individu a au moins un descendant et donc $q = 0$. 
  \item On voit ainsi que la probabilité d'extinction vaut $q = 1$ si le processus est sous-critique ($m < 1$) ou sous-critique ($m = 1$, puisque $\Pr\{X = 0\} > 0$). 
...
$$
% $q$ est donc bien un point fixe de $f$. Le lemme \ref{lem:BGWnombrePointsFixes} indique que leur nombre dépend de la position de $m$ par rapport à 1. Il reste montrer que $q$ est le plus petit d'entre eux, noté $s^*$. \\
...
% q_{n+1} = f_{Z_{n+1}}(0) = f_X \circ f_{Z_n} (0) = f_X(\Pr_1\{Z_n = 0\}) = f_X(q_n).
% $$
La convergence de la suite $(q_n)_{n \geq 0}$ vers le plus petit point fixe s'obtient en remarquant que $f_X$ est une fonction croissante et que
$$
...
\end{tabular}
$$
La convergence est donc démontrée pour $Z_0$ aléatoire, avec $q_0 = \Pr\{Z_0 = 0\} \in [0, 1)$ quelconque. Le cas $Z_0 = 1$ correspond au cas $q_0 = 0$.
\eproof
